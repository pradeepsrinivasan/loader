...
import com.flipkart.perf.util.BlockUntilFreePolicy;
import com.flipkart.perf.util.CountedNamedThreadFactory;
...
public class GroupController {
    ...
    private ThreadPoolExecutor groupControllerPool;
...
//        }
        ...
        groupControllerPool =  new ThreadPoolExecutor(group.getThreads(), group.getThreads(), 1, TimeUnit.MINUTES,
                new LinkedBlockingQueue<Runnable>(1),
                new CountedNamedThreadFactory(groupName + "-Group-Controller"), new BlockUntilFreePolicy());
    ...
    public void stop() {
        groupControllerPool.shutdownNow();
    }
    ...
    public boolean isAlive() {
        ...
        for ( SequentialFunctionExecutor sfe :  sequentialFEs) {
            if ( ! sfe.isOver() ) {
                return true;
            }
        }

        if ( ! groupControllerPool.isShutdown()) {
            stop();
        }
